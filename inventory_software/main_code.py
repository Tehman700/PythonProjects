...
def todays():
    ...
    # Function to update right table and inventory
    def update_right_table():
        nonlocal total_sale, total_wholesale_value  # To update the total_sale variable in the outer scope
        selected_item = tree.selection()
        if not selected_item:
            return
        item_id = tree.item(selected_item)["values"][0]
    ...
    # Function to calculate profit (this is just a placeholder)
    def profit_calc():
        # You need to define how profit is calculated, e.g., from sales data
        profit = total_sale - total_wholesale_value # Example: assuming profit is 10% of total sales
